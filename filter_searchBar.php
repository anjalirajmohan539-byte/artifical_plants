...
$days = $row['DeliveryDays'] ?? 4;
  ...
  $books_html .='    </svg>';
  $books_html .='    <a href=';
  $books_html .=($row['cartStatus'] == 0 || $row['ProductId'] == "") ? "vase_action.php?cartId=".($row['Id'])."&categoryId=".$categoryId."&productId=".($row['Id']) : "customer_cart.php";
  $books_html .=' style="user-select: none;"><span class="button-text">';
  $books_html .= ($row['cartStatus'] == 0 || $row['ProductId'] == "") ? "Add To Cart" : "Go To Cart";
  $books_html .= '</span>';
  $books_html .= '</a>';
  $books_html .='   </button>';
