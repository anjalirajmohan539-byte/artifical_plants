...
if(isset($_POST['remove']) && $_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        $removeUpdate = "UPDATE `wishlist` SET `Favorite`= 0 WHERE `CustomerId`= $customer AND Id = $row";
        var_dump($removeUpdate);
        $check = mysqli_query($conn,$removeUpdate);
...
<div class="container wishlist">
    <?php
    $select = "SELECT ap.`Id`, wl.Id AS wishlistId, `ProductImage`, `ProductName`,ap.`Price`, off.DiscountValue, pc.Categorys AS `CategoryId`, wl.CreateDate,ct.ProductId AS CartId,
                    ...
                    LEFT JOIN cart ct ON ct.ProductId = ap.Id
                    ...
                    WHERE wl.`IsDelete` = 0 AND wl.CustomerId = $Id AND wl.Favorite = 1";
                    // var_dump($select);
...
?>
        <tbody>
            <tr>
                ...
                <td style="font-size: 20px;font-weight:bold;">₹<?php echo $finalPrice;?><br><?php if(($finalTotalPrice) != $itemTotalPrice){ ?>
    ...
    <?php } ?></td>
                ...
                <td>
                <button class="wooden-cart-button" title="Add To Cart" <?php if($details['CartId'] != 0){echo "disabled";}?>>
    ...
    ></path>
                ...
                <td class="remove">
                    ...
                    <form action="wishlist.php" method="post">
                        ...
                        <input type="hidden" name="rowId" value="<?php echo $details['wishlistId'];?>">
                                      <button class="wooden-cart-button" title="Remove" type="submit" name="remove" onclick="return confirm('Are you sure to remove this product ?')">
