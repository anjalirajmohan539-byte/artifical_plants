...
<link href="css/test.css" rel="stylesheet">
    <div class="container main">
        ...
        <main class="checkout-form">
            ...
            <section class="card">
                  <?php
            $select="SELECT `Email` FROM `customer_details` WHERE `IsDeleted` = 0 AND `CustomerId` = $Id";
            $check = mysqli_query($conn,$select);

            if(mysqli_num_rows($check)>0)
                {
                    $mail = mysqli_fetch_assoc($check);
                
            ?>
                <h2 class="section-title"><span class="step-number">1</span> Contact Information</h2>
                <div class="form-group">
                    <label for="email">Email Address</label>
                    <input type="email" id="email" value="<?php echo $mail['Email'];?>">
                </div>
                <?php }?>
            </section>
